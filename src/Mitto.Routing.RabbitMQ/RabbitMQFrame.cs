...
namespace Mitto.Routing.RabbitMQ {
	...
	public class RabbitMQFrame {
		...
		public RabbitMQFrame(byte[] data) { _arrByteArray = data; }
		public RabbitMQFrame(RabbitMQFrameType pType, string pQueueID, byte[] data) {
			var arrConnectionID = Encoding.ASCII.GetBytes(pQueueID);
		...
		public RabbitMQFrameType FrameType {
			get { return (RabbitMQFrameType)_arrByteArray.ElementAt(0); }
		}
